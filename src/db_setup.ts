...
export function setupDatabaseTables() {
    ...
    con.query('CREATE TABLE IF NOT EXISTS `reminders` (`reminderid` INT AUTO_INCREMENT, `reminderdatetime` DATETIME NOT NULL, `createddatetime` DATETIME NOT NULL, `channelid` VARCHAR(20) NOT NULL, `userid` VARCHAR(20) NOT NULL, `remindertext` VARCHAR(300) NOT NULL, `rolepingid` VARCHAR(20), `action` VARCHAR(50), PRIMARY KEY (`reminderid`))', function (err: any, result: any, fields: any) {
        if (err) throw err
